...
steps {
          ...
          slackSend (color: "add8e6", message: 'Calculator-munit-failure-mule4 Deployment Started')
          buildsrc() 
          slackSend (color: "0000ff", message: 'Calculator-munit-failure-mule4 Build Sucessfully')
...
stage('upload to atifactory') {
  ...
  "files": [
    {
      "pattern": "**/*.jar",
      "target": "generic-local/mule4-demo-jars_$BUILD_NUMBER/Calculator-munit-failure-mule4.jar"
    }
 ]
}"""                 
              def buildInfo1 = server.upload spec: uploadSpec
                      slackSend (color: "#FFA500",message: 'Calculator-munit-failure-mule4 Uploaded Sucessfully')
            }
    }
}  	
}
   post {
      failure {
           ...
           slackSend (color: "#FF0000",message: 'Calculator-munit-failure-mule4 Deployment Failed')
        }
      success {
...
''', subject: 'Jenkins ${BUILD_STATUS} [#${BUILD_NUMBER}] - ${PROJECT_NAME} ${ENV, var="GIT_URL"}', to: 'devops@eaiesb.com'    
          slackSend (color: "#32CD32", message: 'Calculator-munit-failure-mule4 Deployment is Sucessful')
        }
  }
}
// steps
